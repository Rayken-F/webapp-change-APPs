...
/* DS App Shell RC v3｜2026-08-19
   - Compact module context bar
   - Never shrink shell baseline because of iOS keyboard animation
   - Daily Report reporter auto-fill remains, duplicate reporter card removed
   - Redundant legacy sandblast backup entry hidden
   - Test path only; production ds-app / ds-report-pwa untouched
*/
(function installDsRcV3(){
  ...
  function syncRcViewport(){
    ...
    setKeyboard(false);
    // Important RC v3 rule: outside orientation change, the known full-screen
    // baseline may grow but must never shrink because keyboard-closing events on
    // iOS can briefly report a smaller fake viewport and strand the nav above a gap.
  ...
  window.addEventListener("pageshow",()=>setTimeout(syncRcViewport,60),{passive:true});
  document.addEventListener("visibilitychange",()=>{if(!document.hidden)setTimeout(syncRcViewport,80);},{passive:true});
  if(vv){
  ...
  function injectDailyStyle(doc){
    if(doc.getElementById("ds-shell-rc3-style")) return;
    const style=doc.createElement("style");
    style.id="ds-shell-rc3-style";
    style.textContent=`
  ...
  function installChildKeyboardHooks(frame,doc){
    if(!doc||doc.documentElement.dataset.dsRc3KeyboardHooks==="1") return;
    doc.documentElement.dataset.dsRc3KeyboardHooks="1";
    doc.addEventListener("focusin",function(){setTimeout(syncRcViewport,40);setTimeout(syncRcViewport,180);},true);
    doc.addEventListener("focusout",function(){setTimeout(syncRcViewport,60);setTimeout(syncRcViewport,260);setTimeout(syncRcViewport,600);},true);
  }
  ...
  if(host){
    const observer=new MutationObserver(function(mutations){
      mutations.forEach(function(m){
        if(m.type==="childList") m.addedNodes.forEach(function(node){if(node&&node.tagName==="IFRAME")node.addEventListener("load",()=>setTimeout(()=>{patchFrame(node);updateContextBar();syncRcViewport();},70));});
      });
      setTimeout(patchAllFrames,40);
    });
    observer.observe(host,{childList:true,subtree:false,attributes:true,attributeFilter:["class"]});
    window.__dsRc3ModuleObserver=observer;
  }

  document.addEventListener("click",function(e){
    if(!e.target.closest("[data-nav]")) return;
    setTimeout(()=>{syncRcViewport();updateContextBar();patchAllFrames();},40);
    setTimeout(()=>{syncRcViewport();updateContextBar();patchAllFrames();},240);
  },true);

  let retries=0;
  patchTimer=setInterval(function(){retries+=1;patchAllFrames();syncRcViewport();if(retries>=12)clearInterval(patchTimer);},650);

  writeGeometry();
  updateContextBar();
  window.__DS_RC_V3__={version:"DS_SHELL_IOS_UX_RC_V3_20260819",getState:()=>({stableHeight,visualHeight:readVisualHeight(),candidateHeight:readCandidate(),keyboardOpen,scale:Number(vv&&vv.scale||1)}),resync:syncRcViewport,repatch:patchAllFrames};
})();
