...
(async()=>{
 ...
 ok('no success toast before the save transaction completes',!await page.locator('#iqc31SaveNotice').isVisible());
 ...
 const notice=page.locator('#iqc31SaveNotice'),noticeStarted=Date.now();await notice.waitFor({state:'visible'});
 const box=await notice.boundingBox();
 ok('confirmed save shows a small centered non-blocking status toast',await notice.textContent()==='歸類保存成功'&&box.width<=230&&box.height<=70&&Math.abs(box.x+box.width/2-201)<2&&Math.abs(box.y+box.height/2-437)<2&&await notice.evaluate(e=>getComputedStyle(e).pointerEvents)==='none'&&await notice.getAttribute('role')==='status');
 await page.screenshot({path:path.join(artifacts,'classification-saved-toast.png')});
 await notice.waitFor({state:'hidden',timeout:3500});
 ok('success toast dismisses itself after about 2.5 seconds',Date.now()-noticeStarted>=2000&&Date.now()-noticeStarted<3400);
 ...
 ok('failed save does not show a success toast',!await notice.isVisible());
