/* RC31.17 / IQC-W5-20260926. Loaded before intake/legacy click handlers. */
(function(){
  "use strict";
  const BUILD="RC31.17 / IQC-W5-20260926",DB="ds_iqc_image_rc_v1",ACTIVE="ds_iqc_image_rc_active_batch";
  ...
  function claim(kind){
    ...
    if(kind==="submission"){actionMessage="正在處理測試批次…";progress("正在核對／處理測試批次，請查看下方送出結果。");}
  ...
  function installUi(){
    ...
    if(!$("iqc31Tools")){
      ...
      tools.innerHTML='<strong>RC31.17 / IQC-W5-20260926</strong><p>可一次加入多張或分次補照片。辨識中請保持此頁開啟；切到背景會停止並保留照片。初次使用需下載辨識核心與英數字模型。</p><button id="iqc31Cancel" class="iqc-rc-btn" type="button">停止本輪辨識</button><details><summary>辨識紀錄</summary><p>紀錄不含帳密、照片或 CTN；保留最近 100 個處理事件。</p><button id="iqc31Copy" class="iqc-rc-btn" type="button">複製辨識紀錄</button><textarea id="iqc31LogText" readonly rows="7" style="width:100%;box-sizing:border-box;font-size:12px" aria-label="辨識紀錄"></textarea></details>';
    ...
    const heading=panel.querySelector(".iqc-rc-top h2");if(heading&&heading.textContent!=="📷 Honeywell 影像 RC31.17")heading.textContent="📷 Honeywell 影像 RC31.17";
