...
(function(){
  ...
  const week=7*24*60*60*1000,prefix='ds_iqc_history_photo_cleanup_v1:';
  let running=false,history=false,retryAt=0,checkTimer=null,lastAccount='',notice='';
  function account(){const c=window.DS_PORTAL_BRIDGE?.getSessionContext?.();return c?.token&&c.profile?.permissions?.daily_report_enabled===true?c.profile.user?.account||'':'';}
