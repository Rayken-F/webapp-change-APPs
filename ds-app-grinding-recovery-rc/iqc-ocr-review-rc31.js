/* RC31.3 photo review. Does not change OCR text or submit IQC records. */
(function(){
  "use strict";
  ...
  function photoButtons(){
    $("iqcRcPhotoList")?.querySelectorAll("[data-photo-delete]").forEach(del=>{
      ...
      const count=model.candidates(p).length;button.disabled=controller().isBusy();
      ...
      const status=p.status==="PROCESSING"?"處理中":p.status==="NEEDS_REVIEW"?"未找到 CTN，可重試或人工補登":p.status==="LOCAL_FAILED"?`未完成（${p.localFailure||"辨識中斷"}）`:p.status==="RECOGNIZED"?(count?`已讀出 ${count} 個 CTN 候選`:"辨識已完成，但未找到 CTN"):"等待辨識";
  ...
  function render(){
    ...
    const hint=$("iqcRcCommitHint");if(hint)hint.textContent="RC31.3：逐筆核對 RT／CTN／狀態／廠區／總量；本輪正式 IQC 寫入維持鎖定。";
  }
