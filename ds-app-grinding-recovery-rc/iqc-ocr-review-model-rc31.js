...
(function(root,factory){
  ...
  function candidates(photo){
    ...
    Object.entries(photo.rc31Review?.ctns||{}).forEach(([original,review])=>{
      const row=map.get(original)||{original,ctn:original,stale:true};map.set(original,{...row,...review,manual:true});
    });
    return [...map.values()];
  }
  function build(photos,legacyDecisions=[]){
    ...
    photos.forEach(photo=>candidates(photo).forEach(raw=>{
      ...
      const key=row.rt?[row.rt,row.status,row.plant].join("|"):"UNASSIGNED|"+photo.id;
      ...
      if(!g.photoIds.includes(photo.id))g.photoIds.push(photo.id);if(!g.photoSeqs.includes(row.seq))g.photoSeqs.push(row.seq);
    ...
    groups.forEach(g=>{g.warnings=[...new Set(g.warnings)];g.ready=!!g.rt&&!!g.status&&!!g.plant&&g.expected>0&&g.ctns.length===g.expected&&!g.warnings.length;});
    return groups;
  }
  function updateReview(photo,selection,meta,{clear=false}={}){
    ...
    if(selection.some(x=>!available.has(x.original)))throw new Error("照片候選已變更，請重新開啟手動歸類。");
    ...
    const before={};selection.forEach(x=>{before[x.original]=ctns[x.original]||null;if(clear)delete ctns[x.original];else ctns[x.original]={...values,ctn:clean(x.ctn),at};});
    const proposed=all.map(x=>ctns[x.original]?.ctn||x.original);if(!clear&&new Set(proposed).size!==proposed.length)throw new Error("修改後與同張其他 CTN 重複，請先核對。");
