...
function install(){
 ...
 async function capability(){
  let config={};try{config=JSON.parse(localStorage.getItem('oqc_production_config_v1')||'{}');}catch(_){}
  ...
  const k='client-production-20260918-'+(hash>>>0).toString(16);
  if(k!==T.getKey()||!/^https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/exec$/.test(endpoint))throw Error('請先完成正式 OQC 連線驗證');
  ...
  try{
   ...
   if(!reply.ok||reply.environment!==D.VERSION||reply.batchRemovalPatch!==R.PATCH||reply.rtMasterPatch!=='20260915-rtlist-h2')throw Error('OQC 後端版本不符，尚未移除批次，請聯絡管理者。');
   capabilityKey=k;
