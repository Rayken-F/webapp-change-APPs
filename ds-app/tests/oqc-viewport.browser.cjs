...
(async()=>{await new Promise(r=>server.listen(8778,'127.0.0.1',r));const browser=await(process.env.DS_WEBKIT?webkit:chromium).launch({headless:true,...(!process.env.DS_WEBKIT&&process.env.DS_BROWSER_EXECUTABLE?{executablePath:process.env.DS_BROWSER_EXECUTABLE}:{})});try{
 ...
 else {data={ok:true,environment:D.VERSION};if(['health','profile'].includes(b.api)){await connectionReady;Object.assign(data,{productionEnabled:true,productionBuild:'OQC-PROD-20260918-01',backendVersion:'0.2.3',rtMasterPatch:D.RT_PATCH,batchRemovalPatch:'RM1-20260916',rtGatePatch:'RT-G1-20260916',actor:'測試'});}
