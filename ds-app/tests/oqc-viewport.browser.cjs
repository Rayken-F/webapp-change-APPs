...
const docs={},requests=[],errors=[],results=[];let releaseHome,releaseConnection,homeReceived=false;
const homeReady=new Promise(r=>releaseHome=r);
const connectionReady=new Promise(r=>releaseConnection=r);
...
(async()=>{await new Promise(r=>server.listen(8778,'127.0.0.1',r));const browser=await(process.env.DS_WEBKIT?webkit:chromium).launch({headless:true,...(!process.env.DS_WEBKIT&&process.env.DS_BROWSER_EXECUTABLE?{executablePath:process.env.DS_BROWSER_EXECUTABLE}:{})});try{
 ...
 else {data={ok:true,environment:D.VERSION};if(['health','profile','bootstrap'].includes(b.api)){await connectionReady;Object.assign(data,{productionEnabled:true,productionBuild:'OQC-PROD-20260918-01',backendVersion:'0.2.3',rtMasterPatch:D.RT_PATCH,batchRemovalPatch:'RM1-20260916',rtGatePatch:'RT-G1-20260916',bootstrapPatch:'OQC-BOOTSTRAP-K9-20260920',actor:'測試',docs:Object.values(docs)});}
 ...
 check('login starts one read-only OQC preload without creating a frame',requests.filter(x=>x==='bootstrap').length===1&&!requests.includes('batch_sync')&&await page.locator('iframe[data-module-key="oqc"]').count()===0,requests.slice());
 ...
 await page.locator('#navMore').click();await page.getByRole('button',{name:/OQC 庫存/}).click();const oqc=page.frameLocator('iframe[data-module-key="oqc"]');await oqc.locator('#scanInput').waitFor();await oqc.locator('#newBatch').waitFor({state:'visible'});
 await page.waitForFunction(()=>document.querySelector('iframe[data-module-key="oqc"]')?.contentDocument?.querySelector('#syncState')?.textContent);
 const pendingText=await oqc.locator('#syncState').textContent();check('unconnected OQC never claims server save',pendingText.includes('尚未完成連線確認')&&!pendingText.includes('已保存至'),pendingText);
 check('OQC setup explains data preparation with no extra login', (await oqc.locator('#environment').textContent()).includes('正在準備 OQC 資料'));
 check('entering before preload resolves shares one read and sends no writes',requests.filter(x=>x==='bootstrap').length===1&&!requests.some(x=>['health','profile','batches','batch_sync'].includes(x)),requests.slice());
 releaseConnection();await page.waitForFunction(()=>{const f=document.querySelector('iframe[data-module-key="oqc"]');return f?.contentWindow.OqcDemoTest?.getConnectionState().ready;});
 const oqcFrame=page.frames().find(f=>f.url().includes('/DS-OQC-SHIPPING/'));
 await oqcFrame.evaluate(()=>OqcDailyBatchD1.check());check('daily initialization reuses prepared batches without a second startup read',requests.filter(x=>x==='bootstrap').length===1&&!requests.some(x=>['health','profile','batches'].includes(x)),requests.slice());const previousBatch=await oqcFrame.evaluate(async()=>(await OqcDemoTest.getRoot()).active);await oqc.locator('#newBatch').click();await page.waitForTimeout(200);check('manual new batch button changes active batch',previousBatch!==(await oqcFrame.evaluate(async()=>(await OqcDemoTest.getRoot()).active)));await oqc.locator('#scanInput').fill('FD56WAE');await oqc.locator('#scanBtn').click();await page.waitForTimeout(1300);
 ...
 }finally{releaseHome();releaseConnection();await browser.close();server.close();}})().catch(e=>{releaseHome();releaseConnection();console.error(e.stack);server.close();process.exitCode=1;});
