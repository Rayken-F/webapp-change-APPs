...
(function(){
  ...
  const endpoint=window.IqcProduction.endpoint;
  const active=()=>localStorage.getItem(window.IqcProduction.key("ds_iqc_image_rc_active_batch"))||'',ctl=()=>window.__DS_IQC_RC31,$=id=>document.getElementById(id);
  ...
  let current=null,busy=false,epoch=0,preview=null,message='送出後將寫入正式 IQC。',messageBatch='',reading=null,historyKey='',historyPhotos=false,phase='',started=0,ticker=null;
  function context(){const c=window.DS_PORTAL_BRIDGE?.getSessionContext?.(),account=c?.profile?.user?.account;
    ...
    if(c.profile.permissions?.iqc_image_enabled!==true)throw Error('此帳號未開啟IQC 影像辨識權限。');return {token:c.token,account};}
  ...
  async function refresh(){
    ...
    const n=++epoch;const promise=(async()=>{const next=id?await store.snapshot(id):null;
      if(n!==epoch||id!==active())return;if(current?.batch?.id!==id){messageBatch='';message='送出後將寫入正式 IQC。';}if(preview&&preview.batchId!==id)closePreview();current=next;paint();
  ...
  function paint(){
    ...
    text(commit,busy?'正在處理目前批次…':'預覽並送出目前批次');disabled(commit,!configured()||working||!ready||!current.photos.length||frozen());
  ...
  async function post(body,account,timeout=60000){
    ...
    try{const r=await fetch(endpoint,{method:'POST',cache:'no-store',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({...body,session_token:c.token}),signal:controller.signal});
      ...
      if(data.protocol!==model.protocol||data.environment!==model.environment)throw Error('回應不是指定正式影像後端，已停止處理。');ctl().submissionTiming(body.api,Date.now()-began,data.server);return data;
    }finally{clearTimeout(timer);}
  }
  async function accept(data,r){
    if(!model.receipt(data,r))return false;
    stage('已收到收據，正在保存歷史');await store.update(r,{status:'SYNCED',receipt:data.receipt,lastError:''});phase='';note(`正式 IQC已寫入 ${data.receipt.rowCount} 筆，已列入歷史紀錄｜${data.receipt.writtenAt}｜收據 ${data.receipt.receiptId}`);return true;
  }
  ...
  async function openPreview(){
    if(!configured()){note('獨立正式影像後端尚未設定，資料保留本機。');return;}
    ...
    await operation(async()=>{
      context();const snapshot=await store.snapshot(active());let legacy={};try{legacy=JSON.parse(localStorage.getItem(window.IqcProduction.key("ds_iqc_v8_meta_override_")+active())||'{}');}catch(_){}
      ...
      panel.innerHTML=`<h3>本批送出預覽（正式 IQC）</h3><p>區域 ${esc(snapshot.batch.regionCode)}｜${data.items.length} 支｜重複 ${data.duplicateCount} 筆已合併</p>${warning.length?'<ul class="iqc-issue">'+warning.map(w=>'<li>'+esc(w)+'</li>').join('')+'</ul>':''}${table(data.items)}<label style="display:flex;gap:10px;align-items:flex-start;margin:16px 0"><input id="iqc31SubmitConfirm" type="checkbox" style="width:22px;height:22px;flex:none"><span>已逐一核對 CTN 字元、歸屬與實際數量；確認送至正式 IQC。</span></label><div class="iqc-rc-row"><button id="iqc31SubmitAccept" type="button" class="iqc-rc-btn good" disabled>確認送出正式批次</button><button id="iqc31SubmitBack" type="button" class="iqc-rc-btn">返回修改</button></div>`;
  ...
  async function commit(){
    ...
    await operation(async()=>{
      ...
      const c=context(),p=model.payload(saved.snapshot,saved.data,'IQCIMG_PROD_'+crypto.randomUUID());
