...
(function installIqcHybridSecondReaderV21(){
  ...
  const SOURCE_DB=window.IqcProduction.key("ds_iqc_image_rc_v1");
  const SOURCE_DB_VERSION=1;
  const HYBRID_DB=window.IqcProduction.key("ds_iqc_hybrid_rc_v15");
  const HYBRID_DB_VERSION=1;
  const ACTIVE_BATCH_KEY=window.IqcProduction.key("ds_iqc_image_rc_active_batch");
  ...
  function sourcePhotos(batchId){if(window.__DS_IQC_RC31?.readPhotos)return window.__DS_IQC_RC31.readPhotos(batchId);return openSourceDb().then(db=>new Promise((resolve,reject)=>{const tx=db.transaction("photos","readonly"),req=tx.objectStore("photos").index("batchId").getAll(batchId);req.onsuccess=()=>resolve((req.result||[]).sort((a,b)=>Number(a.seq)-Number(b.seq)));req.onerror=()=>reject(req.error);tx.oncomplete=()=>db.close();}));}
  function sourcePhoto(photoId){if(window.__DS_IQC_RC31?.readPhoto)return window.__DS_IQC_RC31.readPhoto(photoId);return openSourceDb().then(db=>new Promise((resolve,reject)=>{const tx=db.transaction("photos","readonly"),req=tx.objectStore("photos").get(photoId);req.onsuccess=()=>resolve(window.__DS_IQC_IMAGE_RC?.restorePhoto?.(req.result)||req.result||null);req.onerror=()=>reject(req.error);tx.oncomplete=()=>db.close();}));}
  function putSourcePhoto(photo){if(window.__DS_IQC_RC31?.writePhoto)return window.__DS_IQC_RC31.writePhoto(photo);return openSourceDb().then(db=>new Promise((resolve,reject)=>{const tx=db.transaction("photos","readwrite");tx.objectStore("photos").put(window.__DS_IQC_IMAGE_RC?.storedPhoto?.(photo)||photo);tx.oncomplete=()=>{db.close();resolve(photo);};tx.onerror=()=>{db.close();reject(tx.error);};}));}
