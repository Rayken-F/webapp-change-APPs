...
/* RC31.12 photo review. Does not change OCR text or submit IQC records. */
(function(){
  "use strict";
  ...
  function render(){
    ...
    const conflicts=view.conflicts.length?'<section id="iqc31Conflicts" class="iqc-group bad"><strong>CTN 歸屬待核對</strong><p class="iqc-rc-note">以下 CTN 的來源資料有衝突或歸屬不明，只在此列一次；尚未決定正確歸屬，請依提示核對。</p>'+view.conflicts.map(c=>'<div class="iqc-issue bad"><strong class="iqc31-conflict-ctn">'+esc(c.ctn)+'</strong><p>'+esc(c.reason)+'</p><ul>'+c.choices.map(x=>'<li>RT '+esc(x.rt)+'｜'+esc(x.status||'狀態待填')+'｜'+esc(x.plant||'廠區待填')+'</li>').join('')+'</ul><div class="iqc-rc-row">'+c.photoIds.map(id=>'<button type="button" class="iqc-rc-btn" data-review-photo="'+esc(id)+'">核對第 '+snapshot.find(p=>p.id===id)?.seq+' 張</button>').join('')+'</div></div>').join('')+'</section>':'';
  ...
  async function refresh(list){
    const id=++refreshId,batch=localStorage.getItem(window.IqcProduction.key("ds_iqc_image_rc_active_batch"));
    const photos=Array.isArray(list)?list:await controller().readPhotos();
    if(id!==refreshId||batch!==localStorage.getItem(window.IqcProduction.key("ds_iqc_image_rc_active_batch")))return;
    snapshot=photos;const next=batch+'|'+JSON.stringify(photos.map(p=>[p.id,p.updatedAt,p.status,p.ocrText,p.rc31Review]));
    if(next!==signature){signature=next;let legacy={};try{legacy=JSON.parse(localStorage.getItem(window.IqcProduction.key("ds_iqc_v8_meta_override_")+batch)||'{}');}catch(_){}groups=model.build(photos,model.legacyDecisions(photos,legacy));render();}
