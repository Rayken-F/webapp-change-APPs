...
/* IQC V1.0 / 20260927. Loaded before intake/legacy click handlers. */
(function(){
  "use strict";
  const BUILD="IQC V1.0 / 20260927",DB=window.IqcProduction.key("ds_iqc_image_rc_v1"),ACTIVE=window.IqcProduction.key("ds_iqc_image_rc_active_batch");
  ...
  let operation=null,sequence=0,currentPhoto=0,trace=[],uiTimer,queuedStart=null,refreshSequence=0,viewSignature="",progressBatch="";
  ...
  async function photoTransaction(mode,work,storeName="photos"){const db=await openDb();return new Promise((resolve,reject)=>{
    let value,tx,timeout=false;try{tx=db.transaction(storeName,mode);}catch(_){db.close();reject(fail("STORAGE_ERROR"));return;}
  ...
  async function put(photo){const stored=await photoCodec.encode(photo);await photoTransaction("readwrite",(s,done,tx)=>{const r=tx.objectStore("batches").get(photo.batchId);r.onsuccess=()=>{try{if(!r.result||r.result.status!=="DRAFT"){tx.abort();return;}s.put(stored);}catch(_){tx.abort();}};},["photos","batches"]);photoFailures.delete(photo.id);}
  ...
  function claim(kind){
    ...
    const run={id:++sequence,kind,batch:activeBatch(),started:Date.now(),cancelled:false,abort:new AbortController()};operation=run;
    if(kind==="review"){actionMessage="正在保存歸類…";progress("正在保存歸類，完成後會自動恢復按鈕。");}
    if(kind==="submission"){actionMessage="正在處理正式批次…";progress("正在核對／處理正式批次，請查看下方送出結果。");}
  ...
  async function exclusive(run,work){
    if(navigator.locks)return navigator.locks.request("ds-iqc-ocr-production-v1",{ifAvailable:true},async lock=>{if(!lock)throw fail("OTHER_TAB_BUSY");check(run);return work();});
  ...
  async function finish(run){
    ...
    else if(run.kind!=="review")refresh().catch(e=>record({stage:"refresh",outcome:"error",code:e.code||"STORAGE_ERROR"}));
  }
  function requestStart(photoId){
    if(window.__DS_IQC_SUBMIT31?.frozen()){progress("本批已有送出紀錄，請查收據／重試；新增資料請使用新批次。");return;}
  ...
  async function ingest(files){
    ...
    });progress(`已保存 ${saved} 張照片，可按「開始辨識」。`);}
  ...
  function paint(){
    ...
    text("iqcRcOcrBadge",operation?({local_ocr:"本機辨識中",save_photos:"保存照片中",cloud:"補辨識中"}[operation.kind]||"處理中"):"V1.0｜本機辨識");
  }
  function installUi(){
    const button=$("iqcRcAnalyze"),panel=$("iqcImageRc");if(!button||!panel)return;
    // Keep the legacy DOM hook for compatibility; only the camera-section action is exposed.
    button.hidden=true;
    if(!$("iqc31StartTop")){
      const start=document.createElement('button');start.id='iqc31StartTop';start.type='button';start.className='iqc-rc-btn good';const actions=$("iqcRcGalleryBtn").parentElement;actions.classList.add('iqc31-photo-actions');actions.appendChild(start);
    ...
    const heading=panel.querySelector(".iqc-rc-top h2");if(heading&&heading.textContent!=="📷 Honeywell 影像 V1.0")heading.textContent="📷 Honeywell 影像 V1.0";
  ...
  async function persistReviews(run,updates){
    // Metadata for current byte-backed photos is updated directly, without
    // decoding/re-encoding an image or rereading all photos for every UI refresh.
  ...
  document.addEventListener("click",e=>{
    ...
    if(target.id==="iqcRcCommit"){window.__DS_IQC_SUBMIT31?.openPreview();return;}
    if(target.id==="iqcRcSyncPending"){window.__DS_IQC_SUBMIT31?.retry();return;}
    ...
    if(target.id==="iqcRcNewBatch"){if(window.__DS_IQC_REVIEW31?.allowLeave()!==false)editBatch(()=>api().newBatch()).catch(()=>{});return;}
    if(target.dataset.photoDelete){
      const run=claim("edit_batch");exclusive(run,()=>api().deletePhoto(target.dataset.photoDelete)).catch(()=>progress("批次更新失敗，請稍後重試。")).finally(()=>finish(run));
    }
  },true);
  ...
  setInterval(()=>{if(operation?.task)updateLive();else if(operation?.kind==="review"){actionMessage=`正在保存歸類… ${Math.floor((Date.now()-operation.started)/1000)} 秒`;updateLive();}},1000);
